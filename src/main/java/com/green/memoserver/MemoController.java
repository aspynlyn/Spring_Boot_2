...
public class MemoController {
  ...
  @PostMapping
  public ResultResponse<Integer> postMemo(@RequestBody MemoPostReq req){ // RequestBody를 쓰지 않으면 기본적으로 ModelAttribute가 자동으로 붙어 html의 form태그를 통해 넘어오는 정보로 명시하게 됨
    log.info("req={}", req);
  ...
  // Read (get) /api/memo
  @GetMapping
   public ResultResponse<List<MemoGetRes>> getMemo(@ModelAttribute MemoGetReq req){// 변수명이 쿼리스트링의 key명과 같아야 value가 넘어옴
...
//           ex).newparam();
  ...
  @GetMapping("{id}")
  public ResultResponse<MemoGetOneRes> getMemoById(@PathVariable int id){
    log.info("id={}", id);
